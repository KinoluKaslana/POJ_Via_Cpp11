...
思路其实很简单，就是对小时候的竖式加法进行一个模拟，由于我是以复习为目的，所以采用了灵活度相对较低的迭代器来对vector进行数字的加减
其中核心函数就是_vec_plus_vec两个大数相加，同时也是最容易发生迭代器失效的地方；以及str_p_str两个数相乘，这两个函数
其中，两数相乘会调用两数相加以，每三个数相乘以后的两组数值后面的一组会*10（模拟加减反）之后调用两数相加。
*/
